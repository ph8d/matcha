...
exports.init = http => {
	...
	io.on('connection', async (client) => {
		...
		client.on('leave conversation', (conversation_id, user_id) => {
		...
		client.on('send message', async (data) => { // Maybe i can refacor this
			try {
				...
				io.in(conversationRoom).clients(async (err, clients) => {
					...
					if (clients.length < 2) {
						const authorProfile = await Profile.findOne({ user_id: client.user_id});
